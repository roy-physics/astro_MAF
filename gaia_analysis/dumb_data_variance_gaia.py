...
data = np.load('./../../gaia_data_filtered.npy').astype(np.float32)
...
threshold = 1000 # should remove the high velocity stars
...
# Save the Initial State, NOTE: I'VE ALREADY DONE THIS BUT IF IT'S NOT ALREADY DONE, THEN WILL NEED TO UNCOMMENT THIS PART
...
# Assuming the input is in the form "python train.py [1000,2000,3000]"
arg = sys.argv[1]
...
#preprocessing
# remove the mean of the data with shape (n_samples, 6) where 6 is the dimension of the data
...
# select random indices for validation dataset
np.random.seed(41)
valid_indices = np.random.choice(data.shape[0], 1000000, replace=False)
arr_pos_prep_valid = data[valid_indices]
mean_valid = arr_pos_prep_valid.mean(axis=0)
print(f"mean_valid: {mean_valid}")
arr_pos_prep_valid = arr_pos_prep_valid - mean_valid # subtract the mean
...
# now save the non-validation data from data for use in training
data_train = np.delete(data, valid_indices, axis=0)
...
for train_size in train_sizes:
    ...
    for i in range (0,n_flow):
        ...
        # select random indices for training dataset which are not in the validation dataset
        np.random.seed(dataseeds[i])
        train_indices = np.random.choice(data_train.shape[0], train_size, replace=False)
        arr_pos = data_train[train_indices]
        ...
        arr_pos = arr_pos - np.mean(arr_pos,axis=0) # subtract the mean
        ...
        path_temp_wgt = "./wgt_maf_"+str(train_size)+"_data_gaia"#path_temp_wgt = path_temp_wgts[i]
        path_log_file = "./wgt_maf_"+str(train_size)+"_data_gaia.log"#path_log_file = path_log_files[i]
        ...
        with open(path_log_file, 'w') as fp:
            ...
            np.save(str(train_size)+'_datavariance_losses_gaia.npy',losses) # updates and saves best loss at the end of every flow training loop
    ...
    #np.save(str(train_size)+'_datavariance_losses.npy',losses)
